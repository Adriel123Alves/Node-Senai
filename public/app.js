...
// 1. Função que centraliza a digitação
...
function criarTecladoVirtual() {
    ...
    teclas.forEach(teclaTexto => {
        ...
        botao.setAttribute('id', `tecla-${teclaTexto}`);
        
        if (teclaTexto === 'ENTER') {
            botao.innerText = 'ENTER';
            botao.classList.add('tecla-larga');
        } else if (teclaTexto === 'BACKSPACE') {
            botao.innerText = '⌫';
            botao.classList.add('tecla-larga');
        } else {
            botao.innerText = teclaTexto;
        }

        botao.addEventListener('click', () => {
            botao.blur(); // Tira o foco do botão após o clique
            processarEntrada(teclaTexto);
        });
